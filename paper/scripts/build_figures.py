...
Run from paper/ root:  python scripts/build_figures.py
"""
...
RESULTS = os.path.join(_PAPER_ROOT, "data", "eval")    # eval JSONs
FIGURES = os.path.join(_PAPER_ROOT, "figures")         # output
...
def fig_pareto() -> None:
    ...
    fig.tight_layout()
    out = os.path.join(FIGURES, "fig_entropy_pareto.png")
...
def fig_router_consolidation() -> None:
    ...
    fig.tight_layout()
    out = os.path.join(FIGURES, "fig_router_consolidation.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
...
def fig_dense_vs_sparse() -> None:
    ...
    # Δ annotation and bottom note removed — caption carries this information

    fig.tight_layout()
    out = os.path.join(FIGURES, "fig_dense_vs_sparse_quality.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved: {out}")
